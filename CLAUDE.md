...
Unit test (JVM): `./gradlew testDebugUnitTest`. Coprono `PlaybackQueue`, `AesGcmStreams` (cifratura a stream), `TransferCrypto` (handshake ECDH/HKDF/SAS), il manifest di trasferimento, `PlaylistRemapper` e — per l'esportazione WebDAV — `LibraryTree` (nomi e albero virtuale) e `WebDavXml` (encoding degli href); non esistono test strumentati, quindi la logica critica va tenuta fuori dalle API Android per restare testabile su JVM.
...
- **domain/webdav/** — esportazione della libreria verso un PC: `WebDavServer` (server WebDAV **read-only** Ktor CIO sulla LAN, capability URL con token), `LibraryTree` (albero virtuale `Artista/Album/NN - Titolo.ext`, logica pura), `WebDavXml` (risposte `207 Multi-Status`, logica pura)
- **ui/** — Compose screens with ViewModels. Navigation: `TrackList` → `TrackDetail/{trackId}`, `PlaylistList`, `PlaylistDetail`, `AuthorList`, `AuthorDetail`, `Stats`, `AppInfo`, `FtpConfig`, `FtpSync`, `WifiUpload`, `SendLibrary`, `ReceiveLibrary`, `WebDavExport`. App entry gated by `BiometricGateScreen`
...
- Esportazione su PC (LAN, WebDAV): la schermata "Esporta su PC" apre `WebDavServer` su una porta di `8101..8110` sotto un token casuale e mostra i comandi rclone da incollare. Il PC lista con `PROPFIND` e scarica con `GET`; **il confronto su cosa manca lo fa il client**, guardando la propria cartella (`rclone copy --size-only`, o `rclone mount --read-only` + `rsync --ignore-existing`), quindi l'app non tiene alcun indice di ciò che è già stato esportato. L'albero è una vista su `tracks` con cache a 60 s; `getcontentlength` è `length() - 28` calcolato dal file cifrato — deve coincidere al byte con quanto serve la `GET`, o rclone aborta. La `GET` decifra in streaming con `decryptTo`; i metodi di scrittura rispondono `405`
...
`res/xml/network_security_config.xml` consente il traffico HTTP in chiaro: serve al lato client del trasferimento, che parla con un IP privato non elencabile in una network security config. Lì i payload sono comunque cifrati a livello applicativo; **l'esportazione WebDAV invece serve audio in chiaro**, vedi *Notes*.
...
- Esportazione WebDAV su PC: solo LAN, app in primo piano (nessun foreground service, coerente con FTP, upload Wi-Fi e trasferimento); server **read-only**. A differenza del trasferimento fra istanze **i brani viaggiano in chiaro**: dall'altra parte c'è rclone, che di un handshake ECDH non saprebbe che farsene — la protezione è la stessa dell'upload Wi-Fi (capability token, bind sulla LAN, finestra limitata alla schermata aperta). Chiudendo la schermata le nuove connessioni vengono rifiutate subito, mentre una risposta già in volo arriva a termine. La stabilità dei nomi è il contratto verso il client: gli omonimi si disambiguano con un suffisso derivato dall'ID del brano, mai con un contatore, altrimenti il PC riscaricherebbe tutto a ogni giro. Documentazione della feature in `docs/features/002-esportazione-libreria-webdav/`.
- **Trappola di prestazioni con AES-GCM su Android**: il provider AEAD accumula l'input fino a `doFinal`, quindi il costo cresce col **numero** di chiamate a `update`, non solo con i byte. `CipherInputStream` legge a blocchi di 512 byte e su un file da 7 MB genera ~14.000 update → comportamento quadratico (~1,9 MB/s misurati). In `AesGcmStreams` le letture sono perciò accorpate in blocchi da 64 KB (`fill()`) prima di ogni `update`, e chi deve alimentare la rete usa `transcodeTo` (trasferimento fra istanze) o `decryptTo` (esportazione WebDAV) invece di `CipherInputStream`: stesso file, ~60-95 MB/s (misurati 97-123 MB/s su emulatore). **Attenzione a `decryptingStream`: è `CipherInputStream`** — il `BufferedInputStream` da 64 KB che ha sotto risparmia le syscall, non le `update`, quindi non va usato per servire file interi. Vale per qualsiasi nuovo codice che cifri a stream, incluse le letture dalla rete (una `read` su socket può restituire pochi KB).
